...
const Employees = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:8080/employee/findEmplyee');
  ...
  const handleSave = async (userId) => {
    try {
      const response = await fetch(`http://localhost:8080/employee/updateInfo3`, {
        method: 'POST',
        headers: {
  ...
  const handleEdit = (user_id, username, name, phone,
  ) => {
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input
                ...
                placeholder="Search by Username or Phone..." // Adjusted placeholder text
                required
              />
          ...
          <table className="w-full text-md bg-white shadow-md rounded mb-4">
            <tbody>
              <tr className="border-b">
                ...
                <th className="text-left p-2 px-3">Chức vụ</th>
                <th className="text-left p-2 px-3">Phone</th>
                {/* Add other table headers based on your API response */}
                <th></th>
              </tr>
              {filteredUsers.map((user) => (
                <tr key={user.user_id} className="border-b hover:bg-orange-100">
                  ...
                  <td className="p-2 px-3">
                    <input
                      type="text"
                      value={user.name}
                      onChange={(e) => handleInputChange(e, user.user_id, 'name')}
                      className="bg-transparent border-b-2 border-gray-300 py-2"
                    />
                  </td>
                  {
                    user.role == "ROLE_PT" ? (
                      <td className="p-2 px-3">
                        <input
                          type="text"
                          value="PT"
                          onChange={(e) => handleInputChange(e, user.user_id, 'numberSession')}
                          className="bg-transparent border-b-2 border-gray-300 py-2"
                        />
                      </td>
                    ) : (
                  ...
                  <td className="p-2 px-3">
                    <input
                      type="text"
                      value={user.phone}
                      onChange={(e) => handleInputChange(e, user.user_id, 'phone')}
                      className="bg-transparent border-b-2 border-gray-300 py-2"
                    />
                  </td>
                  ...
                  <td className="p-2 px-3 flex justify-end">
                    <button
                      type="button"
                      onClick={() => handleEdit(user.user_id, user.username, user.name, user.phone
                      )}
